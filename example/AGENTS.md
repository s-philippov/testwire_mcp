...
This is a Flutter app used as a **testwire** demo. The app has a home screen
with a **Leave Review** section (feedback form).
...
| `integration_test/demo_incremental_test.dart` | **Demo 2 — Incremental Development Flow.** Starts with 2 steps; rest are commented out. |
...
**The setup:** The test has 6 steps. Step 5 (Verify success screen) has a
...
3. Check state (`get_test_state`) — step 5 should be `FAIL`.
...
9. Check state (`get_test_state`) — step 5 should now be `PASS` or `FIXED`.
...
**The setup:** The test file has 2 active steps (Navigate to Leave Review,
Enter name). Four more steps are commented out below the `TODO(agent)` marker.

**What you must do:**

1. Connect to the running test (`connect` tool with the VM Service URI).
2. Run remaining (`run_remaining`) to execute the available steps.
3. Check state (`get_test_state`) — steps 0-1 should be `PASS`. The test is
   now paused with no more steps to run.
...
7. Check state (`get_test_state`) — all 6 steps should be `PASS`.
