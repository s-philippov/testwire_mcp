...
/// isolate (subsequent calls are no-ops).  Typically called in `main()`:
///
/// ```dart
/// void main() {
...
/// - [TestwireExtension.disconnect] -- disconnect agent
/// - [TestwireExtension.notifyHotReload] -- signal a hot reload
void registerTestwireExtensions() {
  if (_extensionsRegistered) return;
  _extensionsRegistered = true;

  developer.registerExtension(TestwireExtension.getState.method, (
    method,
    parameters,
  ) async {
    final json = activeSession.registry.toJsonString();
    return developer.ServiceExtensionResponse.result(json);
  });

  developer.registerExtension(TestwireExtension.stepForward.method, (
    method,
    parameters,
  ) async {
  ...
  developer.registerExtension(TestwireExtension.runRemaining.method, (
    method,
    parameters,
  ) async {
  ...
  developer.registerExtension(TestwireExtension.retry.method, (
    method,
    parameters,
  ) async {
  ...
  developer.registerExtension(TestwireExtension.disconnect.method, (
    method,
    parameters,
  ) async {
  ...
  developer.registerExtension(TestwireExtension.notifyHotReload.method, (
    method,
    parameters,
  ) async {
    activeSession.notifyHotReload();
    return developer.ServiceExtensionResponse.result(
      ExtensionResponse.notifyHotReload.encode(),
    );
  });
}
